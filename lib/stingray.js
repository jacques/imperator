...
var TM_API_BASE = '/api/tm/2.0/config/active';
var TM_API_POOLS = TM_API_BASE + '/pools';
...
module.exports = function stingray (options) {
  ...
  function getPools() {
    ...
    _req(TM_API_POOLS, function (err, res, data) {
      if (err) {
        debug('ERROR:', err);
  ...
  function getPool(name) {
    ...
    _req(TM_API_POOLS + '/' + encodeURIComponent(name), function (err, res, data) {
      if (err) {
        debug('ERROR:', err);
    ...
    getPool(pool)
      .then(function (data) {
        if (_.contains(data.properties.basic.nodes, node)) {
          ...
          _req(TM_API_POOLS + '/' + encodeURIComponent(pool), options, function (err, res, data) {
            if (err) {
              debug('ERROR:', err);
